...
/**
 * Класс-обертка интерфейса Connection. Это делается для того, чтобы
 * переопределить метод close(). Чтобы тот кто будет использовать
 *  ConnectionPool не мог его закрыть.
 *
...
class ConnectionWrapper implements Connection {
    ...
    /**
     * Метод close() переопределяется для того, чтобы никто не мог закрыть
     * соединение кроме класса ConnectionPool.
     */
    @Override
    public void close() {
    ...
    public boolean setShardingKeyIfValid(ShardingKey shardingKey,
        ...
        return connection.setShardingKeyIfValid(shardingKey, superShardingKey,
                                                timeout);
    }
